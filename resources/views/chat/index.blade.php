...
<div class="pt-24 pb-20 bg-slate-50 min-h-screen flex flex-col">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex-1 flex flex-col">
        ...
        <div id="chat-box" class="bg-slate-100/50 border-x border-slate-200 flex-1 p-6 overflow-y-auto min-h-[400px] flex flex-col gap-4">
            
            <div class="flex justify-center mb-2">
                <span class="text-xs bg-slate-200 text-slate-500 px-3 py-1 rounded-full font-medium">Ruang obrolan aman & terenkripsi</span>
            </div>

            @forelse($chats as $chat)
                @if($chat->sender_id === Auth::id())
                    <div class="flex justify-end">
                        <div class="bg-blue-600 text-white p-4 rounded-2xl rounded-tr-none shadow-sm shadow-blue-200 max-w-[80%]">
                            <p class="text-sm">{{ $chat->message }}</p>
                            <span class="text-[10px] text-blue-200 mt-2 block text-right">{{ $chat->created_at->format('H:i') }}</span>
                        </div>
                    </div>
                @else
                    <div class="flex justify-start">
                        <div class="bg-white border border-slate-200 text-slate-700 p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[80%]">
                            <p class="text-sm">{{ $chat->message }}</p>
                            <span class="text-[10px] text-slate-400 mt-2 block text-right">{{ $chat->created_at->format('H:i') }}</span>
                        </div>
                    </div>
                @endif
            @empty
        ...
        <div class="bg-white rounded-b-3xl border border-slate-200 p-4 shadow-sm">
            <form id="chat-form" action="{{ route('chat.store', $order->id) }}" method="POST" class="flex gap-3 items-end">
                @csrf
                <textarea id="message-input" name="message" rows="1" class="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50 resize-none" placeholder="Ketik pesan atau lampirkan link bukti bayar..." required></textarea>
                <button type="submit" class="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">
