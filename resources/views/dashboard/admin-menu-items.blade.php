            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-6 h-fit">
                    ...
                    <p class="text-sm text-slate-500 mb-5">
                        @if($editMenuItem)
                            Perubahan menu hanya berlaku untuk order baru. Riwayat order lama tetap memakai snapshot saat transaksi.
                        @else
                    ...
                    <form method="POST"
                        ...
                        <div>
                            ...
                            <input type="text" name="nama"
                                value="{{ old('nama', $editMenuItem->nama ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="toko"
                                value="{{ old('toko', $editMenuItem->toko ?? '') }}"
                        ...
                        <div>
                            ...
                            <select name="kategori"
                                ...
                                <option value="makanan" {{ old('kategori', $editMenuItem->kategori ?? 'makanan') == 'makanan' ? 'selected' : '' }}>
                                    Makanan
                                </option>
                                <option value="minuman" {{ old('kategori', $editMenuItem->kategori ?? '') == 'minuman' ? 'selected' : '' }}>
                                    Minuman
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="harga"
                                    value="{{ old('harga', $editMenuItem->harga ?? '') }}"
                                    placeholder="3000"
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($menuItems as $menu)
                                        <tr class="hover:bg-slate-50">
                                            ...
                                            <td class="px-6 py-4">
                                                @if($menu->aktif)
                                                    <span class="px-3 py-1 rounded-full bg-green-50 text-green-700 border border-green-100 text-xs font-bold">
                                                        Aktif
                                                    </span>
                                                @else
                                                    <span class="px-3 py-1 rounded-full bg-slate-50 text-slate-500 border border-slate-100 text-xs font-bold">
                                                        Nonaktif
                                                    </span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex justify-end gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('dashboard.admin.menu-items.toggle', $menu->id) }}">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                    ...
                                                    <form method="POST" action="{{ route('dashboard.admin.menu-items.delete', $menu->id) }}"
