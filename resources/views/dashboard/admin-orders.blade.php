@extends('layouts.app')
@section('title', 'Semua Order - Admin JastipKu')

@section('content')
<div class="pt-16 bg-slate-50 min-h-screen">
    <div class="flex">
        <!-- Sidebar (sama seperti admin.blade.php) -->
        ...
        <main class="flex-1 lg:ml-64 p-6 lg:p-8">
            <div class="flex items-center justify-between mb-8">
                <h1 class="font-display text-2xl font-800 text-slate-900">📦 Semua Order</h1>
                <!-- Filter -->
                <form method="GET" class="flex gap-2">
                    <select name="status" onchange="this.form.submit()" class="text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white focus:border-blue-400 outline-none">
                        <option value="">Semua Status</option>
                        @foreach(['pending','proses','otw','selesai','batal'] as $s)
                        <option value="{{ $s }}" {{ request('status') == $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                    <select name="kategori" onchange="this.form.submit()" class="text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white focus:border-blue-400 outline-none">
                        <option value="">Semua Kategori</option>
                        @foreach(['makanan','atk','obat','fashion','elektronik','minimarket','kado','lainnya'] as $k)
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Kode Order</th>
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Pemesan</th>
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Kategori</th>
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Budget</th>
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Jastiper</th>
                                ...
                                <th class="text-left text-xs font-semibold text-slate-500 px-6 py-4">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-slate-600">{{ $order->jastiper?->name ?? '-' }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <form method="POST" action="/dashboard/admin/orders/{{ $order->id }}/update-status">
                                        @csrf @method('PATCH')
                                        <select name="status" onchange="this.form.submit()" class="text-xs border border-slate-200 rounded-lg px-2 py-1.5 bg-white">
                                            @foreach(['pending','proses','otw','selesai','batal'] as $s)
                            ...
                            @empty
                            <tr><td colspan="8" class="px-6 py-12 text-center text-slate-400 text-sm">Belum ada order</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
