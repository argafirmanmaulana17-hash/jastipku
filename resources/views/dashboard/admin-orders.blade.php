        ...
        <div class="flex">
            ...
            <aside class="w-64 bg-white border-r border-slate-200 fixed left-0 top-16 hidden lg:block"
                ...
                <div class="p-6">
                    ...
                    <nav class="space-y-1">
                        @foreach ([['href' => '/dashboard/admin', 'icon' => '📊', 'label' => 'Overview'], ['href' => '/dashboard/admin/orders', 'icon' => '📦', 'label' => 'Semua Order'], ['href' => '/dashboard/admin/jastipers', 'icon' => '🛵', 'label' => 'Kelola Jastiper'], ['href' => '/dashboard/admin/users', 'icon' => '👥', 'label' => 'Kelola User'], ['href' => '/dashboard/admin/reports', 'icon' => '📈', 'label' => 'Laporan']] as $menu)
                            <a href="{{ $menu['href'] }}"
                ...
                <div class="flex items-center justify-between mb-8">
                    ...
                    <form method="GET" class="flex gap-2">
                        <select name="status" onchange="this.form.submit()"
                            class="text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white focus:border-blue-400 outline-none">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @forelse($orders as $order)
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-sm font-semibold text-slate-800">Rp
                                            {{ number_format($order->budget, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 text-sm text-slate-600">{{ $order->jastiper?->name ?? '-' }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <span
                                        ...
                                        <td class="px-6 py-4">
                                            <form method="POST"
                                                action="/dashboard/admin/orders/{{ $order->id }}/update-status">
                                                @csrf @method('PATCH')
                                                <select name="status" onchange="this.form.submit()"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-6 py-12 text-center text-slate-400 text-sm">Belum ada
                                            order</td>
                                    </tr>
                                @endforelse
                            </tbody>
