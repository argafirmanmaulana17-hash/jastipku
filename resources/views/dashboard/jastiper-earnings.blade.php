            ...
            <main class="flex-1 lg:ml-64 p-6 lg:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    <div class="bg-white border border-slate-200 rounded-2xl p-6">
                        ...
                        <div class="font-display text-2xl font-700 text-blue-600">Rp
                            {{ number_format($totalEarnings, 0, ',', '.') }}</div>
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl p-6">
                        ...
                        <div class="font-display text-2xl font-700 text-slate-900">
                            Rp
                            {{ $orders->count() > 0 ? number_format($totalEarnings / $orders->count(), 0, ',', '.') : '0' }}
                        </div>
                        ...
                        @forelse($orders as $order)
                            <div class="flex items-center justify-between px-6 py-4">
                                <div>
                                    ...
                                    <div class="text-sm text-slate-600">{{ $order->nama }} ·
                                        {{ ucfirst($order->kategori) }}</div>
                                    <div class="text-xs text-slate-400">{{ $order->created_at->format('d M Y, H:i') }}
                                    </div>
                                </div>
                                <div class="text-right">
                                    <div class="font-semibold text-green-600">+ Rp
                                        {{ number_format($order->budget, 0, ',', '.') }}</div>
