...
@section('content')
        ...
        <div class="flex">
            ...
            <aside class="w-64 bg-white border-r border-slate-200 fixed left-0 top-16 hidden lg:block"
                ...
                <div class="p-6">
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-100">
                        <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Status Kamu</div>
                        <form method="POST" action="/dashboard/jastiper/toggle-status">
                            ...
                            {{ ($jastiper->status ?? 'offline') == 'aktif' ? 'border-green-400 bg-green-50' : 'border-slate-200 bg-white' }}">
                                ...
                                {{ ($jastiper->status ?? 'offline') == 'aktif' ? 'text-green-700' : 'text-slate-500' }}">
                                    <span
                                        class="w-2 h-2 rounded-full {{ ($jastiper->status ?? 'offline') == 'aktif' ? 'bg-green-500 animate-pulse' : 'bg-slate-400' }}"></span>
            ...
            <main class="flex-1 lg:ml-64 p-6 lg:p-8">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h1 class="font-display text-2xl font-800 text-slate-900">Halo,
                            {{ Auth::user()->name ?? 'Jastiper' }}! 👋</h1>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    @foreach ([['label' => 'Order Hari Ini', 'value' => $stats['today'] ?? '0', 'icon' => '📅', 'sub' => 'order'], ['label' => 'Total Selesai', 'value' => $stats['total_done'] ?? '0', 'icon' => '✅', 'sub' => 'order'], ['label' => 'Rating', 'value' => ($stats['rating'] ?? '5.0') . '★', 'icon' => '⭐', 'sub' => 'rata-rata'], ['label' => 'Penghasilan Bulan Ini', 'value' => 'Rp ' . number_format($stats['monthly_earn'] ?? 0, 0, ',', '.'), 'icon' => '💰', 'sub' => 'bulan ini']] as $stat)
                ...
                <div class="bg-white rounded-2xl border border-slate-200 mb-6">
                    <div class="p-6 border-b border-slate-100">
                        <h2 class="font-display text-lg font-700 text-slate-800">🔔 Order Menunggu Konfirmasi</h2>
                    </div>
                        ...
                        @forelse($pendingOrders ?? [] as $order)
                            <div class="p-6">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <div class="font-mono text-sm font-semibold text-blue-600">{{ $order->kode_order }}
                                        </div>
                                        <div class="font-semibold text-slate-800 mt-0.5">{{ $order->nama }}</div>
                                    </div>
                                    <span class="text-lg font-bold text-slate-800">Rp
                                ...
                                <div class="bg-slate-50 rounded-xl p-3 text-sm text-slate-600 mb-4">
                                    {{ Str::limit($order->detail_pesanan, 100) }}
                                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200">
                    <div class="p-6 border-b border-slate-100">
                        <h2 class="font-display text-lg font-700 text-slate-800">🛵 Order Aktif Saya</h2>
                    </div>
                    <div class="divide-y divide-slate-50">
                            ...
                            <div class="p-6">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        ...
                                        <div class="font-semibold text-slate-800">{{ $order->nama }}</div>
                                        <div class="text-sm text-slate-500">📱 {{ $order->whatsapp }}</div>
                                    </div>
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold badge-proses">
   ...
   💬 Chat Pembeli
                        ...
                        @empty
                            <div class="p-12 text-center text-slate-400 text-sm">Tidak ada order aktif saat ini</div>
                        @endforelse
                    </div>
                </div>
