    ...
    <div class="flex">
        ...
        <main class="flex-1 lg:ml-64 p-6 lg:p-8">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                @foreach ([
                    ['label' => 'Order Hari Ini', 'value' => $stats['today'] ?? '0', 'icon' => '📅'],
                    ['label' => 'Total Selesai', 'value' => $stats['total_done'] ?? '0', 'icon' => '✅'],
                    ['label' => 'Rating', 'value' => ($stats['rating'] ?? '5.0') . '★', 'icon' => '⭐'],
                    ['label' => 'Penghasilan Bulan Ini', 'value' => 'Rp ' . number_format($stats['monthly_earn'] ?? 0, 0, ',', '.'), 'icon' => '💰'],
                ] as $stat)
                    ...
                    @forelse($pendingOrders ?? [] as $order)
                        <div class="p-6">
                            ...
                            @if($order->jenis_harga === 'pricelist')
                                <div class="bg-green-50 border border-green-200 rounded-xl p-3 text-sm text-green-700 mb-4">
                                    <div class="font-bold mb-1">Harga Price List</div>
                                    <div>Harga barang: Rp {{ number_format($order->harga_barang ?? $order->budget, 0, ',', '.') }}</div>
                                    <div>Ongkos jastip: Rp {{ number_format($order->ongkos_jastip ?? 3000, 0, ',', '.') }}</div>
                                    <div class="font-bold">
                                        Total: Rp {{ number_format($order->total_bayar ?: (($order->harga_barang ?? $order->budget) + 3000), 0, ',', '.') }}
                                    </div>
                                </div>
                            @else
                                <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-700 mb-4">
                            ...
                            <div class="flex gap-3 mt-4">
                                <form method="POST" action="/dashboard/jastiper/orders/{{ $order->id }}/accept" class="flex-1">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                ...
                                <form method="POST" action="/dashboard/jastiper/orders/{{ $order->id }}/reject" class="flex-1">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                    ...
                    @forelse($activeOrders ?? [] as $order)
                        <div class="p-6">
                            ...
                            @if($order->status == 'menunggu_harga')
                                <form method="POST" action="/dashboard/jastiper/orders/{{ $order->id }}/offer-price"
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                        <input type="number" name="harga_barang" required min="1000" placeholder="Harga barang"
                                            class="w-full px-3 py-2 rounded-lg border border-amber-300 text-sm">

                                        <input type="number" name="ongkos_jastip" required min="1000" placeholder="Ongkos jastip"
                                            class="w-full px-3 py-2 rounded-lg border border-amber-300 text-sm">
                                    </div>
                            ...
                            @if(in_array($order->status, ['proses', 'otw']))
                                <div class="bg-green-50 border border-green-200 rounded-xl p-3 text-sm text-green-700 mb-4">
                                    <div>Harga barang: Rp {{ number_format($order->harga_barang ?? $order->budget, 0, ',', '.') }}</div>
                                    <div>Ongkos jastip: Rp {{ number_format($order->ongkos_jastip ?? 0, 0, ',', '.') }}</div>
                                    <div class="font-bold">Total: Rp {{ number_format($order->total_bayar ?: $order->budget, 0, ',', '.') }}</div>
                                </div>
                            @endif

                            <div class="flex flex-wrap gap-2 mt-3">
                                @if($order->status == 'proses')
                                    <form method="POST" action="/dashboard/jastiper/orders/{{ $order->id }}/update-status" class="flex-1">
                                        @csrf
                                        @method('PATCH')
