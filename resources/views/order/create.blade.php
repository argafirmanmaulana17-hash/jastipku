    ...
    <div class="pt-24 pb-20 bg-slate-50 min-h-screen">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-10">
                ...
                <p class="text-slate-500 mt-3">Isi form di bawah dengan lengkap ya, biar jastiper bisa proses ordermu dengan
                    cepat!</p>
            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="nama" value="{{ old('nama', Auth::user()->name) }}"
                                    ...
                                    class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm bg-slate-50 cursor-not-allowed text-slate-500"
                                    readonly>
                            </div>
                            <div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                @if ($savedAddresses->isNotEmpty())
                                    <select id="select_alamat" onchange="pilihAlamatOtomatis(this.value)"
                                        ...
                                        @endforeach
                                        <option value="tambah_baru" class="text-blue-600 font-bold">➕ Gunakan Alamat Baru...
                                        </option>
                                    </select>
                                @endif
                                ...
                                <div id="box_alamat_baru"
                                    ...
                                    <input type="text" name="label_alamat_baru" id="label_alamat_baru"
                                        placeholder="Simpan alamat ini sebagai? (Contoh: Kosan Blok A, Rumah)"
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select name="kategori" id="kategori" required
                                            ...
                                            <option value="atk" {{ old('kategori') == 'atk' ? 'selected' : '' }}>📚 Alat
                                                Tulis & Buku</option>
                                            <option value="obat" {{ old('kategori') == 'obat' ? 'selected' : '' }}>💊
                                                Obat & Kesehatan</option>
                                            <option value="fashion" {{ old('kategori') == 'fashion' ? 'selected' : '' }}>👕
                                                Fashion</option>
                                            <option value="elektronik"
                                                {{ old('kategori') == 'elektronik' ? 'selected' : '' }}>💻 Elektronik
                                            </option>
                                            <option value="minimarket"
                                                {{ old('kategori') == 'minimarket' ? 'selected' : '' }}>🛒 Minimarket
                                            </option>
                                            <option value="kado" {{ old('kategori') == 'kado' ? 'selected' : '' }}>🎁
                                                Kado</option>
                                    ...
                                    <div id="menu-pricelist-box"
                                        ...
                                        <label class="block text-sm font-medium text-slate-700 mb-2">Pilih Menu Price List
                                            *</label>
                                        <select name="menu_item_id" id="menu_item_id"
                                            ...
                                            @foreach ($menuItems as $menu)
                                                <option value="{{ $menu->id }}" data-harga="{{ $menu->harga }}"
                                                    ...
                                                    {{ $menu->nama }} - {{ $menu->toko }} - Rp
                                                    {{ number_format($menu->harga, 0, ',', '.') }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <p class="text-xs text-slate-500 mt-2">
                                            Untuk makanan/minuman, harga mengikuti price list. Ongkos jastip otomatis
                                            Rp3.000.
                                        </p>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-2">Lokasi Pengambilan
                                            *</label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-slate-700 mb-2">Waktu
                                            Dibutuhkan</label>
                                        <select name="waktu"
                                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none transition-all text-sm bg-white">
                                    ...
                                    @foreach ([['value' => 'dana', 'label' => 'DANA', 'icon' => '💙'], ['value' => 'cash', 'label' => 'Cash (COD)', 'icon' => '💵'], ['value' => 'transfer', 'label' => 'Transfer Bank', 'icon' => '🏦']] as $pay)
                                        <label class="cursor-pointer">
                                            ...
                                            <div
                                                ...
                                                <div class="text-sm font-semibold text-slate-700">{{ $pay['label'] }}
                                                </div>
                                            </div>
                                        </label>
                                    @endforeach
                            ...
                            <!-- Catatan -->
                            <div class="mb-8">
                                <label class="block text-sm font-medium text-slate-700 mb-2">Catatan Tambahan
                                    (opsional)</label>
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-8">
                                <div class="flex gap-3">
                                    ...
                                    <div class="text-sm text-blue-700">
                                        <strong>Cara Pembayaran:</strong> Bayar DP 50% saat order dikonfirmasi, sisanya saat
                                        barang diterima. Transfer ke DANA Bisnis JastipKu atau bayar tunai ke jastiper.
                                    </div>
                                </div>
                            </div>
            ...
            <div class="mt-6 text-center">
                <p class="text-sm text-slate-500">Butuh bantuan? <a href="/chat"
                        class="text-blue-600 font-semibold hover:underline">Chat dengan kami</a> atau hubungi WhatsApp <a
                        href="https://wa.me/62812xxxxxxxx"
                        class="text-blue-600 font-semibold hover:underline">0812-xxxx-xxxx</a></p>
            </div>
        </div>
    </div>
@endsection
