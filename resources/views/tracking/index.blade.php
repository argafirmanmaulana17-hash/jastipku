        ...
        <div class="bg-white rounded-2xl shadow-md border border-blue-100 p-6 mb-8 ring-4 ring-blue-50/50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-slate-50 rounded-xl p-4 border border-slate-100">
                    ...
                    <p class="text-xs text-slate-600 mt-3 bg-white p-2.5 rounded-lg border border-slate-200/60 leading-relaxed">
            ...
            <div class="border-t border-slate-100 pt-5">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-7 gap-3 text-center">
                    <div class="p-3 rounded-xl border {{ $order->status == 'pending' ? 'bg-amber-50 border-amber-200 text-amber-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        ⏳ Menunggu
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'menunggu_harga' ? 'bg-orange-50 border-orange-200 text-orange-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        💸 Tunggu Harga
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'menunggu_persetujuan' ? 'bg-yellow-50 border-yellow-200 text-yellow-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        💰 Persetujuan
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'proses' ? 'bg-blue-50 border-blue-200 text-blue-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        👨‍🍳 Diproses
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'otw' ? 'bg-purple-50 border-purple-200 text-purple-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        🏍️ Diantar
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'selesai' ? 'bg-emerald-50 border-emerald-200 text-emerald-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        ✅ Selesai
                    </div>

                    <div class="p-3 rounded-xl border {{ $order->status == 'batal' ? 'bg-rose-50 border-rose-200 text-rose-700 font-bold' : 'bg-slate-50 border-slate-100 text-slate-400' }} text-xs">
                        ❌ Batal
                    </div>
                </div>
            </div>
                            ...
                            <tr class="hover:bg-blue-50/50 cursor-pointer transition-colors {{ request('kode') == $item->id ? 'bg-blue-50/30 font-semibold border-l-4 border-blue-500' : '' }}"
                                ...
                                <td class="p-4">
                                    ...
                                    <div class="text-slate-400 text-xs mt-0.5">
                                        {{ $item->created_at->format('d M Y') }} - {{ ucfirst($item->kategori_item_snapshot ?? $item->kategori) }}
                                    </div>
                                ...
                                <td class="p-4">
                                    @if($item->status == 'pending')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-amber-50 text-amber-700 border border-amber-100">
                                            Menunggu
                                        </span>
                                    @elseif($item->status == 'menunggu_harga')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-orange-50 text-orange-700 border border-orange-100">
                                            Menunggu Harga
                                        </span>
                                    @elseif($item->status == 'menunggu_persetujuan')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-yellow-50 text-yellow-700 border border-yellow-100">
                                            Menunggu Persetujuan
                                        </span>
                                    @elseif($item->status == 'proses')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-blue-50 text-blue-700 border border-blue-100">
                                            Diproses
                                        </span>
                                    @elseif($item->status == 'otw')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-purple-50 text-purple-700 border border-purple-100">
                                            Diantar
                                        </span>
                                    @elseif($item->status == 'selesai')
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
                                            Selesai
                                        </span>
                                    @else
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-slate-100 text-slate-600">
                                            Batal
                                        </span>
                                    @endif
                                </td>
